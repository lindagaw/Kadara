...
class Src_encoded(data.Dataset):
    ...
    def __init__(self, root, train=True, transform=None, download=False):
        ...
        if download:
            ...
            xs_train = torch.from_numpy(np.load( 'snapshots//' + 'src' + '_' + 'train' + '_encoded_xs.npy' ))
            xs_test = torch.from_numpy(np.load( 'snapshots//' + 'src' + '_' + 'eval' + '_encoded_xs.npy' ))
            ys_train = torch.from_numpy(np.load( 'snapshots//' + 'src' + '_' + 'train' + '_encoded_ys.npy' ))
            ys_test = torch.from_numpy(np.load( 'snapshots//' + 'src' + '_' + 'eval' + '_encoded_ys.npy' ))
...
def get_src_encoded(train):
    ...
    src_encoded_dataset = Src_encoded(root=params.data_root,
                        train=train,
                        transform=pre_process,
                        download=True,
                        dataset=dataset)

    src_encoded_data_loader = torch.utils.data.DataLoader(
        dataset=src_encoded_dataset,
        batch_size=params.batch_size,
        shuffle=False)

    return src_encoded_data_loader
