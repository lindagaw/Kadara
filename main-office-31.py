...
if __name__ == '__main__':
    ...
    src_encoder = torch.nn.Sequential(*(list(models.resnet50(pretrained=False).children())[:-1]))
    src_classifier = torch.nn.Linear(2048, 31)
    tgt_encoder = torch.nn.Sequential(*(list(models.resnet50(pretrained=False).children())[:-1]))
