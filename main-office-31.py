...
if __name__ == '__main__':
    ...
    src_data_loader = get_office_31(dataset = 'office-31-webcam', train=True)
    src_data_loader_eval = get_office_31(dataset = 'office-31-webcam', train=False)
    ...
    src_encoder = torch.nn.Sequential(*(list(models.resnet152(pretrained=False).children())[:-1]))
    src_classifier = torch.nn.Linear(2048, 31)
    tgt_encoder = torch.nn.Sequential(*(list(models.resnet152(pretrained=False).children())[:-1]))
    ...
    if os.path.isfile("snapshots//symbiosis-GAN-critic-final.pt") and \
        os.path.isfile("snapshots//symbiosis-GAN-target-encoder-final.pt") and \
        os.path.isfile("snapshots//symbiosis-GAN-target-classifier-final.pt"):
        ...
        tgt_classifier = init_model(tgt_classifier, "symbiosis-GAN-target-classifier-final.pt")
    else:
